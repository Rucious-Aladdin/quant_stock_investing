...
class filter_by_quantile:
    ...
    def get_quantile(self):
        ...
        #분위수 추출
        self.small_cap_index = self.df["market_capitalization"].quantile(q=0.20)
        
        self.pbr_index = self.df[self.df["pbr"] > 0]["pbr"].quantile(q=0.8)
        self.pbr_index_low = self.df[self.df["pbr"] > 0]["pbr"].quantile(q=0.05)
        self.psr_index = self.df[self.df["psr_quarter"] > 0]["psr_quarter"].quantile(q=0.8)
        self.per_index = self.df[self.df["per_quarter"] > 0]["per_quarter"].quantile(q=0.8)
        self.pcr_index = self.df[self.df["pcr_quarter"] > 0]["pcr_quarter"].quantile(q=0.9)

        self.roe_index = self.df[self.df["roe_quarter"] > 0]["roe_quarter"].quantile(q=0.9)
        
        self.debt_rto = self.df[self.df["debt_ratio"] > 0]["debt_ratio"].quantile(q=0.9)
        
        self.ncav_low = self.df["ncav"].quantile(q=0.35)
        self.ncav_high = self.df["ncav"].quantile(q=0.6)
    ...
    def print_quantile(self):
        ...
        print("분위수 출력", end="")
        print("-"*15)
        print("%-20s %6d %-20s" % ("market_cap_index", self.small_cap_index, "하위 20%"))
        print("%-20s %6.2f %-20s" % ("pbr_high", self.pbr_index, "상위 20% (자본잠식 제외)"))
        print("%-20s %6.2f %-20s" % ("pbr_low", self.pbr_index_low, "하위 05% (적자제외)"))
        
        print("%-20s %6.2f %-20s" % ("psr_index", self.psr_index, "상위 20% (적자제외)"))
        print("%-20s %6.2f %-20s" % ("per_index", self.per_index, "상위 20% (적자제외)"))
        print("%-20s %6.2f %-20s" % ("pcr_index", self.pcr_index, "상위 10% (적자제외)"))
        
        print("%-20s %6.2f %-20s" % ("roe_index", self.roe_index, "상위 10% (적자제외)"))
        print("%-20s %6.2f %-20s" % ("debt_index", self.debt_rto, "상위 10% (완전자본잠식제외)"))
